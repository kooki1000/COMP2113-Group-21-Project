...
int showMainMenu(const HighScore& highScore) {
    while (true) {
        ...
        std::cout << "\n";
        std::cout << COLOR_YELLOW << "                    🏆 HIGH SCORE: " << COLOR_BOLD_YELLOW;
        ...
        if (highScore.defeatedDragon) {
            std::cout << COLOR_GREEN << "                    🐉 Dragon Slayer: " << highScore.playerName << COLOR_RESET << "\n";
        }
...
Difficulty selectDifficulty() {
    ...
    while (true) {
        ...
        std::cout << COLOR_DIM << "                     • 150 HP  • More ores  • Fewer enemies\n";
        ...
        std::cout << COLOR_DIM << "                     • 100 HP  • Standard ores  • Standard enemies\n";
        ...
        std::cout << COLOR_DIM << "                     • 75 HP  • Scarce ores  • Many enemies\n";
        std::cout << "                     • 6-letter Wordle  • 10x10 Minesweeper\n";
        std::cout << "                     • Score: 2.0x multiplier\n\n"
                  << COLOR_RESET;

        std::cout << "\n";
        std::cout << COLOR_DIM << "              Use [W/S] or [1-3] to select, [ENTER] to confirm\n";
...
void showHighScores(const std::vector<HighScore>& scores) {
    ...
    std::cout << COLOR_CYAN;
    std::cout << "         ┌──────┬────────────────┬──────────┬────────────┬────────┐\n";
    std::cout << "         │ RANK │     NAME       │  SCORE   │ DIFFICULTY │ DRAGON │\n";
    std::cout << "         ├──────┼────────────────┼──────────┼────────────┼────────┤\n";
    std::cout << COLOR_RESET;
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (i < (int)scores.size()) {
            ...
            std::cout << " " << std::setw(14) << std::left << hs.playerName.substr(0, 14) << " │";
            std::cout << COLOR_YELLOW << " " << std::setw(8) << std::right << hs.score << COLOR_RESET << " │";
...
void showHowToPlay() {
    ...
    std::cout << COLOR_BOLD_CYAN << "    OBJECTIVE:\n"
              << COLOR_RESET;
    std::cout << "    Mine resources, craft better equipment, and defeat the Dragon!\n\n";

    std::cout << COLOR_BOLD_CYAN << "    CONTROLS:\n"
              << COLOR_RESET;
    std::cout << "    ┌─────────────┬────────────────────────────────────────┐\n";
    std::cout << "    │ " << COLOR_YELLOW << "W A S D" << COLOR_RESET << "     │ Move Up/Left/Down/Right                │\n";
    std::cout << "    │ " << COLOR_YELLOW << "SPACE" << COLOR_RESET << "       │ Mine block / Attack                    │\n";
    std::cout << "    │ " << COLOR_YELLOW << "C" << COLOR_RESET << "           │ Open Crafting Menu                     │\n";
    std::cout << "    │ " << COLOR_YELLOW << "I" << COLOR_RESET << "           │ View Inventory                         │\n";
    std::cout << "    │ " << COLOR_YELLOW << "P" << COLOR_RESET << "           │ Pause / Save Game                      │\n";
    std::cout << "    │ " << COLOR_YELLOW << "Q" << COLOR_RESET << "           │ Quit to Menu                           │\n";
    std::cout << "    └─────────────┴────────────────────────────────────────┘\n\n";

    std::cout << COLOR_BOLD_CYAN << "    RESOURCES:\n"
              << COLOR_RESET;
    std::cout << "    " << COLOR_WOOD << "T" << COLOR_RESET << " Wood     → Craft wooden tools (mine stone)\n";
    std::cout << "    " << COLOR_STONE << "#" << COLOR_RESET << " Stone    → Craft stone tools (mine iron)\n";
    std::cout << "    " << COLOR_IRON << "I" << COLOR_RESET << " Iron     → Craft iron tools (mine gold)\n";
    std::cout << "    " << COLOR_GOLD_ORE << "G" << COLOR_RESET << " Gold     → Craft gold tools (mine diamond)\n";
    std::cout << "    " << COLOR_DIAMOND << "D" << COLOR_RESET << " Diamond  → Best equipment!\n\n";
    ...
    std::cout << COLOR_BOLD_CYAN << "    MINIGAMES:\n"
              << COLOR_RESET;
    std::cout << "    • " << COLOR_GREEN << "Wordle" << COLOR_RESET << " - Guess the word to unlock Iron\n";
    std::cout << "    • " << COLOR_YELLOW << "Minesweeper" << COLOR_RESET << " - Clear the grid to unlock Gold\n";
    std::cout << "    • " << COLOR_CYAN << "Space Invaders" << COLOR_RESET << " - Final boss battle!\n\n";

    std::cout << COLOR_DIM << "    Press any key to return to menu..." << COLOR_RESET;
    getch();
}
...
void showGameOver(const GameState& state, bool isVictory) {
    ...
    std::cout << COLOR_CYAN << "                    ║" << COLOR_WHITE;
    std::cout << "  Best Pickaxe:   " << std::setw(15) << getMaterialName(state.player.equipment.pickaxe);
    std::cout << COLOR_CYAN << " ║\n";

    std::cout << COLOR_CYAN << "                    ║" << COLOR_WHITE;
    std::cout << "  Best Armor:     " << std::setw(15) << getMaterialName(state.player.equipment.armor);
    std::cout << COLOR_CYAN << " ║\n";
