 ...
 * If you're adding new structs or enums, put them here so the whole team can
 * use them.
 *
...
struct Inventory {
  ...
  int total() const { return wood + stone + coal + iron + gold + diamond; }
};
...
struct Player {
  ...
  Player()
      : name("Player"), health(100), maxHealth(100), alive(true), facingX(0),
        facingY(1) {}
};
...
struct CraftingRecipe {
  ...
  MaterialTier
      requiredArmor;       // Minimum armor tier required (for armor crafting)
...
struct DifficultySettings {
  ...
  DifficultySettings()
      : name("Normal"), playerHealth(100), scoreMultiplier(1.5f),
        wordleWordLength(5), minesweeperSize(8), minigameDamage(20) {}
};
...
struct HighScore {
  ...
  HighScore()
      : playerName("---"), score(0), difficulty(DIFF_EASY), timestamp(0),
